...
class ParticlePropertiesTab:
    def __init__(self, property_functions):
        self.pfun = property_functions
        
        mass_map
        
        
    def tabulate_mass(self):
        pid2pdg = self._mass_map.pid2pdg
        self._mass = np.zeros(len(pid2pdg), dtype=np.float64)
        
        for pid, pdg in enumerate(pid2pdg):
            self._mass[pid] = self.pfun.get_mass(pdg)
    
    
    def tabulate_ctau(self):
        pid2pdg = self._ctau_map.pid2pdg
        self._ctau = np.zeros(len(pid2pdg), dtype=np.float64)
        
        for pid, pdg in enumerate(pid2pdg):
            self._ctau[pid] = self.pfun.get_mass(pdg)
                
    


class ParticlePropertiesTab11:
